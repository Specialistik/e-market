...
def current_orders(request):
    if request.user.profile:
        ...
        if request.user.profile.role == 'customer':
            cur_orders = Order.objects.filter(customer_id=request.user.id, order_status__in=(1, 3, 5, 8))

        if request.user.profile.role == 'producer':
            cur_orders = Order.objects.filter(producer_id=request.user.id, order_status__in=(1, 3, 5, 8))
